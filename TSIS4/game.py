...
CYAN       = (0,   220, 220)
BLUE       = (30,  100, 255)
SOFT_PINK  = (255, 182, 193)  # Тот самый нежно-розовый
...
def draw_grid(surface):
    # На белом фоне сетку лучше делать очень светлой
    for c in range(COLS):
        x = c * CELL
        pygame.draw.line(surface, (240, 240, 240), (x, HUD_HEIGHT), (x, SCREEN_H))
    for r in range(ROWS):
        y = r * CELL + HUD_HEIGHT
        pygame.draw.line(surface, (240, 240, 240), (0, y), (SCREEN_W, y))

def draw_walls(surface, walls):
    # Убираем проверку на border, если хотим все стены розовыми
    for (c, r) in walls:
        # Используем SOFT_PINK вместо GRAY
        draw_cell(surface, c, r, SOFT_PINK, shrink=1)
